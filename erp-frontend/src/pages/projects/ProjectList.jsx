...
const ProjectList = () => {
  ...
  const { projects, fetchProjects, deleteProject, updateProjectViewPending } =
    useProjectStore();
  ...
  const canAddProjects =
    user?.role === "ERP System Administrator" ||
  ...
  const getClientName = (clientData) => {
    ...
    const client = clients.find(
      (c) =>
        (c._id?.$oid || c._id) === clientId || (c.id?.$oid || c.id) === clientId
    );
  ...
  const getStatusStyle = (status) => {
    const statusMap = {
      ...
      on_hold:
        "bg-gradient-to-r from-orange-50 to-orange-50 text-orange-700 border border-orange-200",
      cancelled:
    ...
    () => [
      {
        ...
        Cell: ({ value, row }) => (
          <button
            onClick={() =>
              navigate(
                `/projects/details/${row.original._id || row.original.id}`
              )
            }
      ...
      },
      {
        ...
        Cell: ({ value }) => (
          <span
            className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold shadow-sm ${getStatusStyle(
              value
            )}`}
          >
            <div className="w-1 h-1 rounded-full bg-current mr-1"></div>
            {value?.replace("_", " ").replace("-", " ").toUpperCase() ||
              "UNKNOWN"}
          </span>
        ),
      },
