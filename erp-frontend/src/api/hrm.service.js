...
export const updateEmployeeData = async (id) => {
  console.log("HUM");
  const response = await api.post(`hrm/employees/user/${id}`);
  console.log(response);
  return response.data;
};
...
export const downloadDocument = async (employeeId, documentId) => {
  try {
    console.log("Service: Starting document download...", {
      employeeId,
      documentId,
    });

    const response = await api.get(
      `hrm/employees/${employeeId}/documents/${documentId}/download`,
      {
        responseType: "blob",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
...
export const getAttendanceFilters = async () => {
  const response = await api.get("hrm/attendance/filters");
  return response.data;
};
...
export const getLeaves = async () => {
  try {
    const response = await api.get("hrm/leaves");
    return Array.isArray(response.data.data.leaves)
...
export const getMyLeave = async () => {
  ...
  // Ensure we return an array
  const leaves =
    response.data?.data?.leaves || response.data?.leaves || response.data || [];
  return Array.isArray(leaves)
    ? { data: { leaves } }
    : { data: { leaves: [] } };
};
export const viewLeave = async () => {
  const response = await api.post(`hrm/leaves/viewLeaveReq`);
  return response;
};
export const getUnreadLeavesCount = async () => {
  console.log("CALLED");
  const response = await api.get(`hrm/leaves/getUnreadLeavesCount`);
  return response.data;
};
