...
exports.createTask = async (req, res) => {
  try {
    ...
    await Employee.updateOne(
      {
        _id: new mongoose.Types.ObjectId(req.body.assigneeId.value),
      },
      { $set: {  taskViewPending: true } }
    );
    ...
    if (task.assignee) {
      try {
        ...
        console.log(
          `Notification created for employee ${task.assignee} for task ${task._id}`
        );
...
exports.updateTask = async (req, res) => {
  try {
    ...
    if (
      ...
      updates.assignee !== oldTask.assignee?.toString()
    ) {
    ...
    if (transformedTask.assignee) {
      try {
        ...
        if (isReassigned) {
          ...
          const changesSummary = updatedFields
            .map((field) => {
              const oldVal = oldTask[field]
                ? oldTask[field].toString()
                : "none";
