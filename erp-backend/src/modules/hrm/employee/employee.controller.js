...
const getAllEmployees = catchAsync(async (req, res) => {
  try {
    ...
    if (req.query.role)
      filter.$or = [{ role: req.query.role }, { roles: req.query.role }];
  ...
  res.status(200).json({
    ...
    data: {
      employee: {
        ...
        roles:
          Array.isArray(updatedEmployee.roles) && updatedEmployee.roles.length
...
const uploadDocument = catchAsync(async (req, res) => {
  ...
  const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
  const filename =
    "document-" + uniqueSuffix + path.extname(req.file.originalname);
...
const updateProfilePicture = catchAsync(async (req, res) => {
  try {
    ...
    const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
    const filename =
      "profile-" + uniqueSuffix + path.extname(req.file.originalname);
