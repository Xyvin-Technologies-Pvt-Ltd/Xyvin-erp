...
exports.createPayroll = async (req, res, next) => {
  try {
    ...
    const netSalary =
      (employee.salary || 0) + totalAllowances - totalDeductions;
...
exports.updatePayroll = async (req, res, next) => {
  try {
    const { allowances, deductions, basicSalary, period, ...updateData } =
      req.body;
    ...
    if (allowances) {
      totalAllowances = Object.values(allowances).reduce(
        (sum, value) => sum + (Number(value) || 0),
        0
      );
    }
    ...
    const netSalary =
      finalBasicSalary +
      Object.values(finalAllowances).reduce(
        (sum, value) => sum + (Number(value) || 0),
        0
      ) -
      Object.values(finalDeductions).reduce(
        (sum, value) => sum + (Number(value) || 0),
        0
      );
