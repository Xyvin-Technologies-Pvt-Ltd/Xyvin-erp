...
exports.getNotification = async (req, res, next) => {
  try {
    ...
    if (notification.user.toString() !== req.user.id) {
      console.error(
        `User ${req.user.id} not authorized to access notification ${req.params.id}`
      );
...
exports.createNotification = async (req, res, next) => {
  try {
    ...
    // logger.info(`Notification created for user ${req.body.user} by ${req.user.name} (${req.user._id})`);
...
exports.markAsRead = async (req, res, next) => {
  try {
    ...
    if (notification.user.toString() !== req.user.id) {
      console.error(
        `User ${req.user.id} not authorized to update notification ${req.params.id}`
      );
      return res.status(403).json({
        success: false,
...
exports.deleteNotification = async (req, res, next) => {
  try {
    ...
    if (notification.user.toString() !== req.user.id) {
      console.error(
        `User ${req.user.id} not authorized to delete notification ${req.params.id}`
      );
      return res.status(403).json({
        success: false,
